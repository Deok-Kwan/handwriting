...
OUTPUT_DIR = './synth_output_mil_test/' # 테스트용 출력 디렉토리 변경
LOG_DIR = os.path.join(OUTPUT_DIR, 'logs/')
METADATA_PATH = './writer_metadata.csv' # 필요시 경로 수정

# Generation Parameters - Multi-Author (Pairs)
MAX_WRITERS = 20       # 테스트용 최대 작성자 수 제한
NUM_DOCS_PER_PAIR = 1  # 작성자 쌍당 생성할 문서 수 (2→1로 감소)
...
NUM_SINGLE_AUTHOR_DOCS_PER_WRITER = 1 # 작성자당 생성할 단일 문서 수 (3→1로 감소)
MIN_LINES_SINGLE_AUTHOR = 1        # 단일 문서 최소 라인 수 (2→1로 변경)
MAX_LINES_SINGLE_AUTHOR = 4        # 단일 문서 최대 라인 수
...
def load_handwriting_data(handwriting_dir, max_writers=None):
    ...
    # 각 작성자의 라인 수 확인
    for wid, paths in writers.items():
        print(f"Writer {wid}: {len(paths)} lines available")
    ...
    # 테스트를 위한 작성자 수 제한
    if max_writers and len(writers) > max_writers:
        limited_writers = dict(list(writers.items())[:max_writers])
        print(f"Limiting to {len(limited_writers)} writers for testing purposes.")
        return limited_writers
...
def generate_documents(writers_data, output_dir, log_dir, metadata_df=None):
    ...
    if GENERATE_SINGLE_AUTHOR_DOCS:
        ...
        if len(writer_ids) == 0:
            ...
            print(f"Starting single-author document generation for {len(writer_ids)} writers")
            ...
            for writer_idx, writer_id in enumerate(writer_ids):
                ...
                print(f"Processing Writer {writer_idx+1}/{len(writer_ids)}: {writer_id} with {len(lines)} lines")
                docs_for_this_writer = 0
                for i in range(NUM_SINGLE_AUTHOR_DOCS_PER_WRITER):
                    try:
                        print(f"  Attempting to generate document {i+1}/{NUM_SINGLE_AUTHOR_DOCS_PER_WRITER} for writer {writer_id}")
                        ...
                        print(f"  Sampling {num_lines} lines for writer {writer_id}")
                        ...
                        if single_img is None: 
                            print(f"  Failed to create image/mask for writer {writer_id}")
                            continue
                        ...
                        print(f"  Writing files for writer {writer_id}: {img_path}")
                        ...
                        if create_log_file(info, log_filename, log_dir):
                            ...
                            print(f"  Successfully created document for writer {writer_id}")
                        else: # Log failed
                            print(f"  Log saving failed for {base_filename}. Removing associated image/mask.")
                            if os.path.exists(img_path): os.remove(img_path)
                            if os.path.exists(mask_path): os.remove(mask_path)
                ...
                if docs_for_this_writer > 0:
                     ...
                     print(f"  Failed to generate any documents for writer {writer_id}.")
    else:
...
if __name__ == "__main__":
    ...
    # 1. Load Handwriting Data - 테스트용 MAX_WRITERS 파라미터 추가
    writers_data = load_handwriting_data(HANDWRITING_DIR, MAX_WRITERS)
